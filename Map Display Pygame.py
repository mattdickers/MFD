import sys, pygame, krpc, time, math, wernher
import numpy as np
...
def DrawMap(mapPos,body,mapType,level,prevOrbit,nextOrbit,color,vessel):
    ...
    try:
        ...
        textFunc("CHEATER",red,(mapPos[0]+imgSize*(levelsDict[level][1]),mapPos[1]+imgSize*(levelsDict[level][0]/8)),"centre",levelsDict[level][5])
        textFunc("Soz the map doesn't like cheated orbits",red,(mapPos[0]+imgSize*(levelsDict[level][1]),mapPos[1]+imgSize*(levelsDict[level][0]/3)),"centre",levelsDict[level][6])
