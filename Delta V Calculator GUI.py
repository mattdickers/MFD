...
def DeltaV(CurrentStage):
    ...
    if len(partStages) == 1:
        ...
        stageMass = sum(mass for Stage in range(stage, -1, -1) for mass in massStages[Stage]) + sum(
            part.mass for part in missingParts)
        ...
        for Stage in range(len(fuelMasses)):
            ...
            for fuel in fuelTypes:
                if vessel.resources_in_decouple_stage(Stage, False).amount(fuel) == 0:
                    ...
                    fuels[fuel] = (vessel.resources_in_decouple_stage(Stage, False).amount(
                        fuel) * vessel.resources.density(fuel))
            fuelMasses[Stage].append(fuels)
        stageDryMass = stageMass - sum(
            mass[fuel] if fuel in fuelMasses[engineDecoupleStages[engineStages[stage][0]]][0] else 0
            for fuel in fuelTypes for mass in fuelMasses[engineDecoupleStages[engineStages[stage][0]]])
        if stageDryMass == stageMass:
            stageDryMass = sum(part.dry_mass for part in partStages[stage]) + sum(
                part.dry_mass for part in partActivateStages[stage]) + sum(part.dry_mass for part in missingParts)
...
while 1:
    ...
    textFunc("Dv: "+str(DeltaV(6)), white, (175, 275), "centre")
